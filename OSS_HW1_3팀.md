...
> #### 5. Termination  
 ...
 5.2. 귀하가 기여자 버전이 특허를 직간접적으로 침해한다고 주장하는 특허 침해 청구(선언적 판결 소송, 반소 및 교차 청구 제외)를 주장하여 모든 법인에 대한 소송을 시작하는 경우, 본 라이선스의 섹션 2.1에 따라 모든 Covered Software에 대해 귀하에게 부여된 권리는 종료된다.

 5.3. 위의 5.1 또는 5.2 항에 따른 종료의 경우, 종료 이전에 귀하 또는 귀하의 배포자가 이 라이선스에 따라 유효하게 부여한 모든 최종 사용자 라이선스 계약(배포자 및 재판매자를 제외한)은 종료 후에도 계속 유효한다.
...
> #### 9. Miscellaneous  
 이 라이선스는 본 주제와 관련된 완전한 합의를 나타냅니다. 이 라이선스의 어떤 조항이 집행 불가능하다고 간주될 경우, 해당 조항은 집행 가능하도록 필요한 범위 내에서만 수정된다. 계약의 문구를 작성자에게 불리하게 해석해야 한다고 규정하는 법률이나 규정은 이 라이선스를 기여자에게 불리하게 해석하는 데 사용되지 않는다.

> #### 10. Versions of the License  
 **10.1. New Versions**  
  Mozilla 재단이 라이선스 관리자다. 섹션 10.3에 명시된 경우를 제외하고 라이선스 관리자 외의 누구도 이 라이선스의 새 버전을 수정하거나 게시할 권리가 없다. 각 버전에는 고유한 버전 번호가 부여된다.
...
&nbsp; Storage Inspector(보관 검사관)는 웹 개발자가 웹 애플리케이션에서 사용하는 다양한 저장소 를 이해, 검사하고 관리할 수 있는 도구이다. 
  ...
  *  Local Storage (로컬 스토리지) :
  ...
  • 웹사이트가 브라우저에 데이터를 저장할 수 있는 방법이다. 키-값 쌍으로 데이 
터를 저장하며, 페이지가 새로 고침되더라도 데이터가 유지된다. 사용자가 설정 
한 정보나 기타 데이터를 영구적으로 저장할 수 있다.
...
> #### 주요 사용법:
  * 컨테이너 탭 활성화 : 먼저 Firefox에서 컨테이너 탭 기능을 활성화해야 한다. 설정 방법은 다음과 같다 : Firefox 주소창에 `about:preferences`를 입력하고 Enter를 누른다.
"개인정보 및 보안" 탭에서 "컨테이너 탭 사용" 옵션을 켜거나, Multi-Account Containers 확장 프로그램을 설치한다.
  * 컨테이너 탭 생성:
컨테이너 탭을 생성하려면, Firefox 탭 바에서 탭 옆의 + 버튼을 길게 누르면 컨테이너 선택 창이 나타난다. 여기서 기본 제공되는 개인, 작업, 금융, 쇼핑 등 다양한 카테고리 중 선택하거나 새로운 컨테이너를 만들 수 있다.
  * 탭 간 세션 격리:
컨테이너 탭을 사용하면 각 탭의 세션, 로그인 정보, 쿠키가 독립적으로 관리되므로 다른 탭에서 다른 계정으로 로그인할 수 있다.
예를 들어, 하나의 컨테이너에서는 Gmail A 계정에 로그인하고, 다른 컨테이너에서는 Gmail B 계정에 로그인할 수 있다.
  * 컨테이너 탭 관리:
컨테이너 탭 관리는 about:preferences 페이지에서 수행할 수 있으며, 원하는 대로 새로운 컨테이너를 만들거나, 기존 컨테이너를 수정할 수 있다.
각 컨테이너에는 고유한 색상과 아이콘을 부여할 수 있어, 시각적으로 구분이 쉽다.
  * 사이트를 특정 컨테이너에서 열기:
특정 사이트를 항상 특정 컨테이너에서 열도록 설정할 수 있다. 예를 들어, Facebook을 "개인" 컨테이너에서만 열리도록 설정하면, 해당 사이트는 항상 지정된 컨테이너에서만 열린다.
 * 장점:
...
 &nbsp; 각 컨테이너는 쿠키, 세션, 로컬 스토리지를 개별적으로 유지된다. 이로 인해 다음과 같은 이점이 제공된다:
  ...
  * 강화된 프라이버시 보호: 각 컨테이너는 데이터를 독립적으로 관리하므로, 웹사이트가 다른 탭의 활동을 추적할 수 없다. 이는 사용자 개인 정보 보호에 큰 도움이 된된다.
...
> #### 주요 기능
  * 데이터 유출 감지 : 
사용자는 자신의 이메일 주소를 등록하여, 해당 이메일이 과거 데이터 유출 사건에 포함되었는지 확인할 수 있다. 만약 유출된 경우, 어떤 웹사이트에서 문제가 발생했는지, 어떤 종류의 데이터가 노출되었는지를 명확히 알려준다.
예를 들어, 비밀번호, 전화번호, 신용카드 정보 등이 유출된 적이 있는지 명확히 파악할 수 있다.
  * 실시간 유출 경고 : 
등록된 이메일이 새로운 데이터 유출 사건에 포함될 경우 즉시 알림을 받아볼 수 있다. 이를 통해 유출 사실을 빠르게 인지하고 비밀번호 변경 등 필요한 조치를 취할 수 있다.
  * 대응 조치 안내 : 
유출이 확인되면, 사용자에게 구체적인 조치를 안내한다. 예를 들어, 비밀번호를 즉시 변경하고 이중 인증을 설정하는 방법을 제공해 보안을 강화할 수 있도록 도와준다.
특히, Firefox Monitor는 단순 경고에 그치지 않고 실제로 사용자가 취해야 할 대응 방법을 구체적으로 제시한다.
  * 다중 이메일 모니터링 : 
사용자는 최대 5개의 이메일 주소를 무료로 등록하여 모두 모니터링할 수 있다. 이를 통해 개인용 이메일 외에도 업무용 이메일 등 다양한 계정을 한 번에 관리하고 보호할 수 있다.

> #### 사용법:
  * 이메일 등록:
Firefox Monitor 웹사이트에 접속하여 이메일 주소를 입력하면, 해당 이메일이 과거에 데이터 유출에 포함되었는지 확인할 수 있다.
이메일을 등록하면, 이후 새로운 유출이 발생할 때마다 경고를 받을 수 있다.
  * 유출 확인 후 조치:
유출이 확인되면, Firefox Monitor는 사용자가 취해야 할 조치(예: 비밀번호 변경, 이중 인증 설정 등)를 안내한다. 이로써 사용자는 신속하게 보안 문제를 해결하고 더 큰 피해를 예방할 수 있다.

<참고!>
현대 사회에서 개인정보 유출은 큰 문제로 대두되고 있다. 특히 이메일 주소는 다양한 온라인 서비스와 연동되기 때문에, 이메일이 유출되면 그 여파는 상당히 클 수 있다. Firefox Monitor는 이러한 위협을 빠르게 인지하고 대응할 수 있는 도구로, 개인의 온라인 보안을 더욱 철저히 지킬 수 있게 해준다.
추가적으로 Firefox Monitor는 Have I Been Pwned와 협력하여, 기존의 대규모 유출 데이터베이스에 접근함으로써 광범위한 데이터 유출 정보를 제공한다. 이를 통해 사용자들이 최신 보안 위협에 대비할 수 있도록 돕는다.
