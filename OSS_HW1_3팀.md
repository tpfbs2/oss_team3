...
실수로 잘못된 변경을 했을 경우, Undo(되돌리기) 및 Redo(다시 실행) 기능을 통해 쉽게 이전 상태로 돌아갈 수 있다.
  ...
  > #### At-규칙 사이드바
&nbsp;  스타일 편집기는 현재 시트에 다음 At 규칙이 포함될 때마다 오른쪽에 사이드바를 표시한다. 사이드바에는 규칙이 나열되며 규칙이 정의된 시트의 행에 대한 링크가 제공된다. 시트에서 해당 규칙으로 이동하려면 항목을 클릭한다. @media 규칙의 경우, 미디어 쿼리가 현재 적용되지 않으 규칙의 조건 텍스트는 회색으로 표시된다. 
