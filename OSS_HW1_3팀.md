...
![image](https://github.com/user-attachments/assets/f81c5292-29a0-4c1a-8e1b-b809209ca82d)
  ...
  > #### 사이드바
&nbsp; 사이드바는 웹 페이지가 사용하는 저장소 유형(예: Cookies, Local Storage, Session Storage, IndexedDB, Cache Storage 등)에 대한 계층적 구조를 제공하여, 사용자가 특정 도메인이나 웹사이트에 관련된 데이터에 쉽게 접근할 수 있도록 한다. 각 항목을 클릭하면 해당 저장소의 세부 정보가 테이블 위젯으로 표시된다. 이를 통해 사용자는 각 데이터 항목을 쉽게 검토하고 조작(수정, 삭제 등)을 할 수 있다. 사이드바는 이러한 데이터 관리 작업을 지원하는 초기 탐색 인터페이스 역할을 한다.

&nbsp; 많은 데이터가 있을 때는 특정 저장소 항목을 필터링하여 필요한 정보를 신속하게 찾을 수 있다.
