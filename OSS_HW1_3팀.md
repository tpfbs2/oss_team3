 ...
 ** 스타일 시트 목록 : 
스타일 시트 창의 왼쪽에는 현재 웹 페이지에 적용된 CSS 파일 목록이 표시된다. 사용자는 이 목록에서 각 스타일 시트를 선택하여 내용을 확인하고 수정할 수 있다. 여러 파일이 로드된 경우, 특정 스타일 시트를 쉽게 찾아볼 수 있도록 구조화되어 있다.

 ** CSS 코드 편집기 :
선택한 스타일 시트의 CSS 코드가 오른쪽 창에 표시됩니다. 사용자는 이 편집기에서 직접 CSS 규칙을 추가, 수정 또는 삭제할 수 있습니다. 변경사항은 실시간으로 웹 페이지에 반영되므로, 디자인 결과를 즉각적으로 확인할 수 있습니다.

 ** 실시간 변경 사항 반영 :
스타일 시트 창에서 수정한 CSS는 실시간으로 웹 페이지에 적용되어, 변경 사항을 즉시 볼 수 있다. 이는 사용자가 스타일을 실험하고 최적의 디자인을 찾는 데 큰 도움이 된다.

 ** CSS 속성 자동 완성 :
속성을 입력할 때 자동 완성 기능이 제공되어, CSS 코드를 작성하는 데 소요되는 시간을 단축할 수 있다.

 ** CSS 규칙 탐색 :
각 CSS 규칙 옆에는 선택할 수 있는 체크박스가 있어, 특정 규칙을 활성화하거나 비활성화할 수 있다. 이를 통해 스타일을 쉽게 켜고 끌 수 있어, 어떤 규칙이 페이지에 어떻게 영향을 미치는지 빠르게 테스트할 수 있다.

 ** 주석 추가 :
개발자는 코드에 주석을 추가하여 특정 스타일의 용도나 변경 사항을 설명할 수 있다. 코드의 특정 부분에 대한 설명이나 메모를 작성해 둘 수 있어, 나중에 이해를 돕거나 팀원과의 협업에 유용하다.

 ** 소스 맵 지원 :
만약 스타일 시트가 전처리기(예: Sass, Less)로 작성된 경우, 소스 맵을 통해 원본 파일로 쉽게 이동할 수 있다. 이를 통해 더 효율적으로 코드를 관리할 수 있다.

 ** 검색 기능 :
스타일 시트 창에는 CSS 규칙을 빠르게 찾을 수 있는 검색 기능이 있어, 긴 코드 속에서 특정 속성이나 클래스를 쉽게 검색할 수 있다. 이 기능은 대규모 프로젝트에서 특히 유용하다.

 ** 파일 저장 :
수정한 CSS는 자동으로 저장되며, 이전 버전으로 쉽게 복원할 수 있는 기능도 있어 작업의 안전성을 보장한다. 사용자는 변경된 사항을 잃지 않고, 필요에 따라 되돌릴 수 있는 유연성을 제공한다.
