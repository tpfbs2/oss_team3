...
&nbsp;  저장소 트리에는 저장소 검사자가 검사할 수 있는 모든 저장소 유형(Cache Storage, Cookies, Indexed DB, Local Storage, Session Storage)이 나열된다.
...
3. 저장소 트리 탐색 : 왼쪽 사이드바에서 원하는 저장소 유형을 선택하여 관련 데이터 항목을 탐색한다.
  ...
  > #### 테이블 위젯
&nbsp; 테이블 위젯에는 선택한 트리 항목(원점 또는 데이터베이스)에 해당하는 모든 항목의 목록이 표시된다. 저장소 유형 및 트리 항목에 따라 테이블의 열 수가 다를 수 있다.
