...
![image](https://github.com/user-attachments/assets/a5e48977-af8a-43ec-9534-86a7540b22a7)
  ...
  > #### 스타일 시트 창
&nbsp;  스타일 시트 창은 웹 페이지에 적용된 CSS 파일을 관리하고 편집할 수 있는 핵심영역이다. 이 창은 웹 개발자와 디자이너가 웹 페이지의 스타일을 실시간으로 확인 및 수정하고, 다양한 CSS 파일을 탐색하며, 스타일 규칙을 빠르게 조정할 수 있도록 도와준다. 또한, 실시간 피드백과 다양한 편집 기능 덕분에 사용자는 더욱 빠르고 효과적으로 웹 페이지의 디자인을 실현할 수 있다. 
...
수정한 CSS는 자동으로 저장되며, 이전 버전으로 쉽게 복원할 수 있는 기능도 있어 작업의 안전성을 보장한다. 사용자는 변경된 사항을 잃지 않고, 필요에 따라 되돌릴 수 있는 유연성을 제공한다.
  ...
  > #### 편집기 창
&nbsp;  우리는 편집기 창에서 선택한 스타일 시트의 출처를 읽고 편집할 수 있다. 변경한 내용은 페이지에 즉시 적용되기 때문에 변경 사항을 쉽게 실험, 수정 및 테스트할 수 있다. 변경 사항이 만족스러우면 스타일 시트 창에서 시트 항목의 저장 버튼을 클릭하여 사본을 로컬에 저장할 수 있다.

  > #### At-규칙 사이드바
&nbsp;  스타일 편집기는 현재 시트에 다음 At 규칙이 포함될 때마다 오른쪽에 사이드바를 표시한다. 사이드바에는 규칙이 나열되며 규칙이 정의된 시트의 행에 대한 링크가 제공된다. 시트에서 해당 규칙으로 이동하려면 항목을 클릭한다. @media 규칙의 경우, 미디어 쿼리가 현재 적용되지 않으 규칙의 조건 텍스트는 회색으로 표시된다. ![image](https://github.com/user-attachments/assets/548f4dc8-ae55-4c11-a667-3dc187e1e4d7)
