...
> #### 컨테이너 탭 사용 방법
&nbsp; 컨테이너 탭을 사용하려면 먼저 필요한 애드온을 설치하거나 기능을 활성화해야 하고, 다음 단계를 통해 시작할 수 있다:
  * 멀티 계정 컨테이너 설치: Mozilla 애드온 웹사이트에서 Multi-Account Containers 확장 프로그램을 설치한다.
  * 새 컨테이너 만들기: 설치 후 툴바에서 컨테이너 아이콘을 클릭하고 새로운 컨테이너 유형(예: 업무용, 개인용, 쇼핑용)을 선택한다.
  * 컨테이너에서 탭 열기: 컨테이너가 생성되면, 특정 컨테이너를 선택한 후 해당 컨텍스트에서 새로운 탭을 열 수 있다. 이렇게 하면 격리된 브라우징 세션을 유지할 수 있다.
